...
function logout(){
	echo "http://".DB_IPADDRESS."/api/logout.php?session=expired";
}
function logout_user(){
	echo "http://".DB_IPADDRESS."/api/logout.php?logout=true";
}
function find_pubname($pubname){
	if ($pubname == "AGENT BOOKLET - BARRY PLANT REAL ESTATE - EMAIL"){
		...
		$output_pub = "GUMTREE.COM.AU";
	} else if($pubname == "FRASER COAST CHRONICLE - SAT"){
		$output_pub = "FRASER COAST CHRONICLE";
	} else {
		$output_pub = $pubname;
	}
	return $output_pub;
