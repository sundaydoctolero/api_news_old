<?php 
	...
	$details = Australia::find_next($session->batch_id,$session->state,$session->publication_name,$session->publication_date);
?>
		...
		<span id="view_address"></span>
			<fieldset>
				...
				<label class="proplabel">Unit No.</label>
					<input class="small" name="unit_no" type="text" value="<?php echo htmlentities($details['unit_no']);?>" pattern="[aA-zZ.0-9-]{1,10}"/><br>
				...
				<label class="proplabel">City</label>
					<input class="medium" id="city" name="city" type="text" value="<?php echo htmlentities($details['city']);?>" readonly /><br>
